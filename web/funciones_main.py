...
def login():
    ...
    if username_bd is False:
        ...
        res = {"status": 200 ,"mensaje":"Login correct", "target":"/dashboard"}
...
def registro():
    ...
    try:
        ...
        if user_exist == True:
            code = 406
            res = {"status": code ,"mensaje":"User exists" }

        else:
            registrar_usuario(requestRegister['username'],requestRegister['password'])
            code = 200
            res = {"status": code , "mensaje":"You have been registered successfully"}

    except:
        code = 406
        res = {"status": code ,"mensaje":"An error has occurred while registering" }
...
def password():
    ...
    if res:
        res={"status": 200 , "mensaje":"Your password has been changed"}
    else:
        res={"status": 406 , "mensaje":"Something went wrong"}
    return res
